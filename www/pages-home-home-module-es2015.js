...
let AcceptPage = class AcceptPage {
    constructor(navCtrl, lp, settings, pop, modal, activatedRoute) {
...
let ChatPage = class ChatPage {
    constructor(navCtrl, plat, alert, modal, eventProvider, ph, lp) {
        this.navCtrl = navCtrl;
        this.plat = plat;
        this.alert = alert;
        this.modal = modal;
...
let HomePage = class HomePage {
    constructor(share, vibration, lp, modalCtrl, storage, settings, One, statusBar, loadingCtrl, router, locationTracker, myGcode, dProvider, platform, // tslint:disable-next-line: no-shadowed-variable
    ...
    starTimer() {
        ...
        this.ph.getUserProfile().on("value", (us) => {
            if (us.val().seconds)
                this.seconds = us.val().seconds;
            if (us.val().minutes)
                this.minutes =
                    us.val().minutes;
            this.ph.getUserProfile().off("value");
            if (us.val().elapse)
                this.noww = us.val().elapse;
            console.log(this.noww);
        });
        this.myVal = Object(rxjs__WEBPACK_IMPORTED_MODULE_8__["interval"])(1000).subscribe(() => {
        ...
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
            const alert = yield this.alert.create({
                message: "You Were Rated. " + name, buttons: [{
                        text: "Okay", role: "cancel", handler: () => { this.ph.doShit3(); },
                    },],
                backdropDismiss: true,
            });
            alert.present();
    ...
    openNews() {
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
            const mod = yield this.modalCtrl.create({ component: _news_news_page__WEBPACK_IMPORTED_MODULE_18__["NewsPage"], });
            yield mod.present();
            mod.onDidDismiss().then(() => {
                this.hideNews = true;
                this.storage.set(`News`, this.news);
            });
            return yield mod.present();
        });
    }
    togglePassengers() {
    ...
    NowWaitingForID() {
        let location_locationTracker_loop = Object(rxjs__WEBPACK_IMPORTED_MODULE_8__["interval"])(3000).subscribe(() => {
    ...
    Register() {
        ...
        this.StartTracker();
        if (this.platform.is("cordova")) {
            this.myListening = Object(rxjs__WEBPACK_IMPORTED_MODULE_8__["interval"])(3000).subscribe(() => {
                this.StartListeningForChanges(this.notify_ID);
            });
        }
        else {
    ...
    StartTracker() {
        ...
        this.watch = this.geo.watchPosition({ enableHighAccuracy: true, timeout: 10000 }).subscribe(position => {
            if (position.coords != undefined) {
                ...
                this.OneSignal.UpdateInfo(this.currLat, this.currLng, this.ph.carType); //push Driver info to be avaolable
            }
            else {
    ...
    OpenInfo() {
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
            ...
            const myAlert2 = yield this.modalCtrl.create({ component: _tripinfo_tripinfo_page__WEBPACK_IMPORTED_MODULE_23__["TripinfoPage"], componentProps: obj, });
            yield myAlert2.present();
            yield myAlert2.onDidDismiss().then((data) => {
    ...
    clearConnection() {
        let customer = firebase__WEBPACK_IMPORTED_MODULE_5___default.a.database().ref(`Customer/${this.notify_ID}`);
    ...
    clearAllNow() {
        ...
        this.locationTracker.hasCompleted = true;
        this.platform.ready().then(() => {
            Object(rxjs__WEBPACK_IMPORTED_MODULE_8__["timer"])(1000).subscribe(() => {
                this.StartListeningForChanges(this.notify_ID);
                this.StartTracker();
    ...
    hasCompletedRide(driverSnap) {
        ...
        if (this.c_complete) {
            ...
            this.ph
                ...
                if (us.val())
                    if (us.val().tipped != null) {
                        ...
                        console.log(this.totalTipped);
                        if (this.eventProvider.historyKey != null) {
            ...
            this.ph
                ...
                this.eventProvider
                    .createHistory(driverSnap.Client_realPrice, driverSnap.Client_username, driverSnap.Client_price, datetime, driverSnap.Client_locationName, driverSnap.Client_destinationName, currentdate.getDay(), driverSnap.Client_Surcharges || 0, toll, driverSnap.Client_OutOfStateCharge, driverSnap.Client_ID, this.totalTipped).then(() => {
                    this.router.navigate(["rate", {
                            eventId: driverSnap.Client_ID, name: driverSnap.Client_username, lat: this.locationTracker.driver_lat, lng: this.locationTracker.driver_lng, cartype: this.ph.carType, positive_Rating: driverSnap.Client_Positive_rating,
                            negative_Rating: driverSnap.Client_Negative_rating, price: driverSnap.Client_price - driverSnap.Client_DriverSurharge, time: this.dProvider.time2, distance: this.dProvider.distance,
                        },]).then(() => {
    ...
    NewClientFound(client_locationName, location, client_name, picture, destinationName, rate, ratingText, price) {
    ...
    gotoMap() {
        ...
        let destination1 = this.locationTracker.driver_lat + "," + this.locationTracker.driver_lng;
    ...
    RejectJob() {
        ...
        this.platform.ready().then(() => {
            ...
            Object(rxjs__WEBPACK_IMPORTED_MODULE_8__["timer"])(1200).subscribe(() => {
                this.canStart = true;
                this.jkl = true;
                this.storage.set("canStart", this.canStart);
                this.storage.set("jkl", this.jkl);
    ...
    createUserInformation(name, picture, id, lat, lng, locationName, driver_bearing, plate, seats, carType, positive, negative, rateText, number) {
        this.eventProvider
            .PushUserDetails(name, picture, id, lat, lng, locationName, plate, carType, positive, negative, rateText, number, this.notify_ID, driver_bearing).then(() => {
            this.locationTracker.map1.clear().then(() => {
                this.locationTracker.yellow_markersArray = [];
                this.locationTracker.driver_markersArray = [];
                this.locationTracker.client_markersArray = [];
    ...
    StartListeningForChanges(id) {
        ...
        if (this.canListen) {
            ...
            this.eventProvider
                ...
                if (this.canStart || this.canStart == null) {
    ...
    FaceShare() {
        let source = [this.locationTracker.driver_lat, this.locationTracker.driver_lng]; // source lat,long
        let destination = [this.destination_lat,
...
let PaymentpagePage = class PaymentpagePage {
    constructor(navCtrl, lp, settings, modal, pop, activatedRoute) {
        this.navCtrl = navCtrl;
        this.lp = lp;
        this.settings = settings;
        this.modal = modal;
...
let TripinfoPage = class TripinfoPage {
    constructor(navCtrl, prof, lp, settings, eventSerEventService, modal, activatedRoute) {
        this.navCtrl = navCtrl;
        this.prof = prof;
        this.lp = lp;
        this.settings = settings;
